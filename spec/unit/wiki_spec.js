...
describe("Wiki", () => {
  beforeEach(done => {
    ...
    sequelize.sync({ force: true }).then(res => {
      User.create({
        ...
        Wiki.create({
          title: "Keyboards",
          body: "If you love keyboards, come here!",
          private: false,
          userId: this.user.id
        }).then(wiki => {
          this.wiki = wiki;
  ...
  describe("#create()", () => {
    it("should create a wiki object with a title,body,privacy status and assigned user", done => {
      Wiki.create({
        title: "Cat Shorts",
        body: "We love clothes made of cats!",
        private: false,
        userId: this.user.id
      })
        .then(wiki => {
          expect(wiki.title).toBe("Cat Shorts");
          expect(wiki.body).toBe("We love clothes made of cats!");
    ...
    describe("getUser", () => {
      it("should return the associated user", done => {
        this.wiki.getUser().then(associatedUser => {
          expect(associatedUser.username).toBe("Paco Smith");
          done();
        });
      });
    });
